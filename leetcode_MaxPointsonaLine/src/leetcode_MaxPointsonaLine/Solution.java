...
public class Solution {
	...
	public Point[] PreProcess(Point[] points)// delete same points, change weights accordingly
	{
		int n = points.length;
		Point[] points2 = new Point[1];
		points2[0] = points[0];
		weight = new int[1];
		weight[0] = 1;
		int[] result;
		for(int i=1; i<n; i++)
		{
			result = Contain(points2, points[i]);
			if(result[0]>0)
			{
				weight[result[1]]++;
			}
			...
			{
				points2 = PushPoint(points2, points[i]);
				weight = PushWeight();
			}
		}
		return points2;
	...
	{
		int[] weight2 = new int[weight.length+1];
		for(int i=0;i<weight.length;i++)
		{
			weight2[i] = weight[i];
		}
		weight2[weight.length] = 1;
		return weight2;
	}
	public Point[] PushPoint(Point[] points2, Point p)
	{
		Point[] points3 = new Point[points2.length+1];
		for(int i=0;i<points2.length;i++)
		{
			points3[i] = points2[i];
		}
		points3[points2.length] = p;
		return points3;
	}
	public int[] Contain(Point[] points2, Point p)//return if points2 contain p and yes then the position of p in points2
	{
